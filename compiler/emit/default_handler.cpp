	...
	namespace compiler
	{
		...
		void DefaultAstHandler::accept(ExpressionAst *node)
		{
			...
			if (node->shouldClearStack)
				addCommand<OpClear>(); // clear expression stack
		}
		...
		void DefaultAstHandler::accept(FunctionDefinitionAst *node)
		{
			...
			for (int i = node->arguments.size() - 1; i >= 0; i--)
			{
				...
				addCommand<VarCreate>(VarType::VAR_TYPE_ANY, str);
				addCommand<StackPopObject>(str, StackType::STACK_FUNCTION_PARAM);
			}
		...
		void DefaultAstHandler::accept(FunctionCallAst *node)
		{
			...
			else if (msg == FN_FOUND)
			{
				for (int i = node->arguments.size() - 1; i >= 0; i--)
				{
					...
					addCommand<OpPush>(StackType::STACK_FUNCTION_PARAM);
		...
		void DefaultAstHandler::accept(IfStatementAst *node)
		{
			...
			addCommand<CreateBlock>(BlockType::IF_STATEMENT_BLOCK,
				blockIdNum++,
				level);
		...
		{
			accept(node->value.get());
			addCommand<OpPush>(StackType::STACK_FUNCTION_CALLBACK);
